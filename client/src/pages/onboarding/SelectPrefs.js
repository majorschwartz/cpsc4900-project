...
const SelectPrefs = ({ stepStage }) => {
	...
	const togglePreference = (item, category) => {
		setSelectedPreferences((prev) => {
			...
			const index = newPreferences[category].indexOf(item.name);
			if (index > -1) {
				newPreferences[category].splice(index, 1);
			} else {
				newPreferences[category].push(item.name);
			}
			
			const allCategoryItems = prefList[category].items.map(i => i.name);
			const allSelected = allCategoryItems.every(i => newPreferences[category].includes(i));
	...
	const handleSavePreferences = () => {
		...
		Object.entries(userAddedItems).forEach(([category, items]) => {
			combinedPreferences[category] = [
				...new Set([...(combinedPreferences[category] || []), ...items])
			];
		});
	...
	const toggleSelectAll = (category) => {
		...
		setSelectedAllCategories(prev => ({ ...prev, [category]: newSelectedAll }));
					...
					([category, { description, items }]) => (
						<div key={category} className="mb-8">
							...
							{(category === "Cuisines" || category === "Meals") && (
								...
								>
									<div className={`w-5 h-5 border-2 rounded mr-2 flex items-center justify-center select-none pointer-events-none ${
										selectedAllCategories[category]
								...
								{items.map((item, index) => (
									<div
										...
										className={`p-4 border rounded-lg cursor-pointer transition-colors duration-100 ${
											selectedPreferences[category]?.includes(item.name)
